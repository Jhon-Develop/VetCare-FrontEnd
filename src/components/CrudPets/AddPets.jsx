...
const AddPets = () => {
    ...
    const [uploadError, setUploadError] = useState(null); // To handle image upload errors
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!user_id) {
                console.error("No user ID (nameid) found in the decoded token");
                return;
            }
            ...
            if (pet.image) formData.append('image', pet.image);
            ...
            console.error("Error adding the pet:", error.response ? error.response.data : error.message);
            alert('There was an error adding the pet. Please try again.');
        }
    };
            ...
            <form onSubmit={handleSubmit} className="w-full md:w-3/4 lg:w-1/2 h-fluid min-h-screen flex flex-col justify-center items-center bg-cWhite rounded-tr-custom rounded-br-custom p-4 md:p-8">
                <div className="flex flex-col items-center justify-center w-full">
                    ...
                    <input
                        ...
                        onChange={handleInputChange}
                        className="w-full md:w-4/5 p-3 md:p-4 h-12 md:h-14 rounded-2xl border border-cPurple text-cBlack bg-cWhite text-sm md:text-base mt-4 md:mt-6"
                        max={new Date().toISOString().split("T")[0]} // Only the date part
                    />
